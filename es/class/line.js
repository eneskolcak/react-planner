  ...
  _createClass(Line, null, [
    {
      ...
      value: function create(state, layerID, type, x0, y0, x1, y1, properties) {
        ...
        var line = state.catalog.factoryElement(
          type,
          {
            id: lineID,
      ...
      value: function remove(state, layerID, lineID) {
        ...
        if (line) {
          ...
          state.getIn(["scene", "groups"]).forEach(function (group) {
            return (state = Group.removeElement(
              state,
              group.id,
              layerID,
              "lines",
    ...
    },
    {
      ...
      value: function split(state, layerID, lineID, x, y) {
        ...
        var splitPointOffset = GeometryUtils.pointPositionOnLineSegment(
          x0,
          y0,
          x1,
          y1,
        ...
        line.holes.forEach(function (holeID) {
          ...
          if (holeOffset < splitPointOffset) {
            ...
            var _offset =
              (holeOffset - splitPointOffset) / (1 - splitPointOffset);
            if (minVertex.x === x1 && minVertex.y === y1) {
              _offset = 1 - _offset;
            }
            state = Hole.create(
          ...
          if (holes) {
            holes.forEach(function (holeWithOffsetPoint) {
              ...
              if (GeometryUtils.isPointOnLineSegment(x1, y1, x2, y2, xp, yp)) {
                var newOffset = GeometryUtils.pointPositionOnLineSegment(
                  x1,
                  y1,
                  x2,
                  y2,
    ...
    },
    {
      ...
      value: function createAvoidingIntersections(
        state,
        layerID,
        type,
        x0,
        ...
        state = state
          ...
          .reduce(function (reducedState, line) {
            var _line$vertices$map$to = line.vertices
            ...
            var intersection = GeometryUtils.twoLineSegmentsIntersection(
              points[0],
              points[1],
              v0,
              v1
            ...
            if (intersection.type === "colinear") {
              ...
              reducedState
                ...
                .forEach(function (holeID) {
                  ...
                  var oldLineLength = GeometryUtils.pointsDistance(
                    v0.x,
                    v0.y,
                    v1.x,
                    v1.y
                  );
                  var offset = GeometryUtils.samePoints(
                    orderedVertices[1],
                    line.vertices.get(1)
                  )
                    ? 1 - hole.offset
                    : hole.offset;
                  var offsetPosition = GeometryUtils.extendLine(
                    v0.x,
                    v0.y,
                    v1.x,
                    v1.y,
        ...
        var _Line$addFromPoints = Line.addFromPoints(
            state,
            layerID,
            type,
            points,
    ...
    },
    {
      ...
      value: function beginDrawingLine(state, layerID, x, y) {
        var snapElements = SnapSceneUtils.sceneSnapElements(
          state.scene,
          new List(),
          state.snapMask
        );
        ...
        var drawingSupport = state
          .get("drawingSupport")
          .set("layerID", layerID);

        state = Layer.unselectAll(state, layerID).updatedState;

        var _Line$create3 = Line.create(
            state,
            layerID,
    ...
    },
    {
      ...
      value: function updateDrawingLine(state, x, y) {
        ...
        var _Line$replaceVertex = Line.replaceVertex(
            state,
            layerID,
            lineID,
            1,
    ...
    },
    {
      ...
      value: function endDrawingLine(state, x, y) {
        if (state.snapMask && !state.snapMask.isEmpty()) {
          var snap = SnapUtils.nearestSnap(
            state.snapElements,
            x,
            y,
            state.snapMask
        ...
        state = state.merge({
          ...
          scene: state.scene.updateIn(["layers", layerID], function (layer) {
            return layer.withMutations(function (layer) {
              ...
              layer.updateIn(
                ["vertices", lineVertices.get(0)],
                function (vertex) {
                  return vertex.merge({ x: newVertex0X, y: newVertex0Y });
                }
              );
              layer.updateIn(
                ["vertices", lineVertices.get(1)],
                function (vertex) {
    ...
    },
    {
      ...
      value: function endDraggingLine(state, x, y) {
        ...
        if (state.snapMask && !state.snapMask.isEmpty()) {
          var curSnap0 = SnapUtils.nearestSnap(
            state.snapElements,
            newVertex0X,
            newVertex0Y,
            state.snapMask
          );
          var curSnap1 = SnapUtils.nearestSnap(
            state.snapElements,
            newVertex1X,
            newVertex1Y,
            state.snapMask
        ...
        if (
          !GeometryUtils.samePoints(
            { newVertex0X: newVertex0X, newVertex0Y: newVertex0Y },
            { newVertex1X: newVertex1X, newVertex1Y: newVertex1Y }
          )
        ) {
          var ret = Line.createAvoidingIntersections(
            state,
            layerID,
            line.type,
            newVertex0X,
    ...
    },
    {
      ...
      value: function setVerticesCoords(
        state,
        layerID,
        lineID,
        x1,
